...
document.addEventListener('DOMContentLoaded', () => {
    if (window.lucide) {
        ...
        if (themeIcon && localStorage.getItem('vitrinedesk_admin_theme') === 'light') {
...
// Theme Management
const savedTheme = localStorage.getItem('vitrinedesk_admin_theme');
if (savedTheme === 'light') {
...
window.toggleTheme = function(btn) {
    ...
    localStorage.setItem('vitrinedesk_admin_theme', next);
    
    const icon = document.getElementById('theme-icon');
    if (icon) {
        icon.setAttribute('data-lucide', next === 'dark' ? 'moon' : 'sun');
        if (window.lucide) {
            // Recria apenas este ícone ou reconstrói todos (mais simples e rápido)
